...
const typename SparseSet::EntityT& SparseSet::_sparseCGet(const IdT id) const noexcept {
	return _sparse[id / Traits::pageSize][id % Traits::pageSize];
}
...
typename SparseSet::EntityT* SparseSet::_sparseTryGet(const IdT id) noexcept {
	...
	return page ? page + id % Traits::pageSize : nullptr;
}
...
const typename SparseSet::EntityT* SparseSet::_sparseTryCGet(const IdT id) const noexcept {
	const size_t pageNum = id / Traits::pageSize;
...
bool SparseSet::containsID(const IdT id) const noexcept {
	const auto sparsePtr = _sparseTryCGet(id);

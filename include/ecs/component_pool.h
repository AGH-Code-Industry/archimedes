...
namespace arch::ecs {
	...
	void ComponentPool<T>::insert(EntityId entity_id, T component) {
		assert_msg(
			_entity_to_index_map.find(entity_id) != _entity_to_index_map.end(),
				   "Entity already has component of this type.");

		size_t new_index = poolSize;
	...
	void ComponentPool<T>::remove(EntityId entity_id) {
		...
		size_t index_of_last_element = poolSize - 1;
	...
	void ComponentPool<T>::on_entity_destroyed(EntityId entity_id) {
		if (_entity_to_index_map.find(entity_id) !=

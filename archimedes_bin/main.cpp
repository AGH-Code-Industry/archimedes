...
int main() {
	...
	for (size_t i = 0; i != 8; ++i) {
		...
		registry.emplace<Pos>(entities.back(), 0.0, 0.0);
		if (i % 2) {
			registry.emplace<Vel>(entities.back(), 0.0, 0.0);
		}
	}

	auto group = registry.group<Vel>(entt::get<Pos>);*/
	...
	domain.addComponent<Flag2>(e3);
	domain.addComponent<Flag>(e6);

	auto view = domain.view<Flag, Flag2>();

	view.forEach([&view](auto entity) {
		auto&& [flag, flag2] = view.getAll(entity);
		std::cout << std::format("{}\n", (uint32_t)entity);
	});

	domain.kill(e6);

	view.refresh().forEach([&view](ecs::e32 entity) {
		auto&& [flag, flag2] = view.getAll(entity);
		std::cout << std::format("{}\n", (uint32_t)entity);
	});

...
std::future<bool> TCPSocket::connectedForce() {
	return std::async(std::launch::async, [this]()->bool {
		return _Base::connectedForce();
	});
}

std::future<bool> TCPSocket::connect(const Host& host, Port port) {
	return std::async(std::launch::async, [this](const Host& h, Port p)->bool {
		return _Base::connect(h.sync(), p);
	}, host, port);
}
std::future<bool> TCPSocket::condConnect(const Host& host, Port port, void* data, int dataLen, int responseLen, AcceptResponseHandler handler, void* handlerData) {
	return std::async(std::launch::async, [this](const Host& h, Port p, void* d, int dl, int rl, AcceptResponseHandler h2, void* hd = nullptr)->bool {
		std::scoped_lock lock(_sendMutex, _recvMutex);
		return _Base::condConnect(h.sync(), p, d, dl, rl, h2, hd);
	}, host, port, data, dataLen, responseLen, handler, handlerData);
}
...
std::future<bool> TCPSocket::condAccept(TCPSocket& new_sock, AcceptCondition condition, int dataLen, int responseLen, void* additionalData) {
	return std::async(std::launch::async, [this](std::reference_wrapper<TCPSocket> ns, AcceptCondition c, int dl, int rl, void* ad)->bool {
		std::scoped_lock lock(_sendMutex, _recvMutex);
		TCPSocket& _ns = ns.get();

		auto acceptFuture = accept(_ns);
		...
		return false;
	}, std::ref(new_sock), condition, dataLen, responseLen, additionalData);
}

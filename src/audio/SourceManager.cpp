...
void SourceManager::play() {
	...
	while (isListening) {
		_updateListener();
		{
			...
			for (auto&& [entity, source] : _domain->view<SourceComponent>().all()) {
				...
				switch (source.getState()) {
					case playing:
						...
						/*} catch (arch::audio::AudioException& e) {
							e.print();
						}*/
					...
					case stopped:
						...
						_stopSource(source, source.removeOnStop);
						if (source.removeOnStop) {
							_removeSource(source);
						}
						break;
					case ignored: /*Logger::debug("Audio system: source is ignored");*/ break;
				}
			}
		}
...
void SourceManager::_stopSource(SourceComponent& component, bool reset) {
	_sources[component.sourceIndex].stopPlaying(reset);

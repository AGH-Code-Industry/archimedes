...
#define FRAME_BEGIN_KEYBOARD(key)                                        \
	if (Keyboard::key._state & KeyState::pressed + KeyState::released) { \
		...
		Keyboard::key._time += diff;                                     \
	}

#define FRAME_BEGIN_MOUSE(key)                                        \
	if (Mouse::key._state & KeyState::pressed + KeyState::released) { \
		Mouse::key._time = {};                                        \
	} else {                                                          \
		Mouse::key._time += diff;                                     \
	}

void System::_frameBegin() noexcept {
	static decltype(Clock::now()) _lastUpdate = Clock::now();
	const auto now = Clock::now();
	const auto diff = now - _lastUpdate;
	_lastUpdate = now;
...
#define FRAME_END_KEYBOARD(key) Keyboard::key._state &= ~(KeyState::pressed + KeyState::released + KeyState::repeat);

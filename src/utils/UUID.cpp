...
static thread_local AutoSeededRandomPool g_prng;
...
UUID UUID::fromName(std::string_view name,
                    const NameBased uuidVersion) noexcept {
  switch (uuidVersion) {
    case NameBased::v3:
      return v3(name);
    case NameBased::v5:
...
UUID UUID::fromNamespacedName(const UUID& nameSpace, std::string_view name,
                              const NameBased uuidVersion) noexcept {
  switch (uuidVersion) {
    case NameBased::v3:
      return v3(nameSpace, name);
    case NameBased::v5:
...
UUID UUID::fromString(std::string_view str) noexcept {
  ...
  if (std::sscanf(str.data(),
                  // clang-format does a little trolling
                  "%02hhx%02hhx%02hhx%02hhx-%02hhx%02hhx-%02hhx%02hhx-%02hhx%"
                  "02hhx-%02hhx%02hhx%02hhx%02hhx%02hhx%"
                  "02hhx",
                  &data[0x0], &data[0x1], &data[0x2], &data[0x3], &data[0x4],
                  &data[0x5], &data[0x6], &data[0x7], &data[0x8], &data[0x9],
                  &data[0xa], &data[0xb], &data[0xc], &data[0xd], &data[0xe],
                  &data[0xf]) != 16) {
    return UUID();
  }
...
uint64_t UUID::_x100nsSinceGregorian() noexcept {
  ...
  return chr::duration_cast<
             chr::duration<long long, std::ratio<1, 10'000'000>>>(
             chr::system_clock::now() -
             chr::sys_days(chr::day(15) / chr::October / chr::year(1'582)))
      .count();
...
UUID UUID::v1(const bool useMAC) noexcept {
  ...
  uint16_t clockSeq;
  g_prng.GenerateBlock(reinterpret_cast<CryptoPP::byte*>(&clockSeq),
                       sizeof(clockSeq));
  ...
  if (useMAC && std::ranges::all_of(_cachedMAC,
    ...
    g_prng.GenerateBlock(&result._data[0xa], 6);
  }
...
UUID UUID::_fromNamespacedName(const UUID& nameSpace, std::string_view name,
                               HashFn hashFn) noexcept {
  UUID result;
  ...
  std::copy(nameSpace._data.data(), nameSpace._data.data() + 16,
            catedData.get());
  ...
  if (hashFn == md5) {
    ...
    md5.CalculateDigest(
        reinterpret_cast<CryptoPP::byte*>(result._data.data()),
        reinterpret_cast<const CryptoPP::byte*>(catedData.get()),
        catedDataSize);
  } else if (hashFn == sha1) {
    ...
    sha.CalculateDigest(
        reinterpret_cast<CryptoPP::byte*>(temp.data()),
        reinterpret_cast<const CryptoPP::byte*>(catedData.get()),
        catedDataSize);
...
UUID UUID::v3(std::string_view name) noexcept { return _fromName(name, md5); }

UUID UUID::v3(const UUID& nameSpace, std::string_view name) noexcept {
  return _fromNamespacedName(nameSpace, name, md5);
}

UUID UUID::v5(std::string_view name) noexcept { return _fromName(name, sha1); }
...
UUID UUID::v4() noexcept {
  ...
  g_prng.GenerateBlock(reinterpret_cast<CryptoPP::byte*>(result._data.data()), result._data.size());
...
UUID UUID::v7() noexcept {
  ...
  uint64_t sinceEpoch = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  g_prng.GenerateBlock(reinterpret_cast<CryptoPP::byte*>(&result._data[0x6]), 10);
...
u32 UUID::version() const noexcept { return _data[0x6] >> 4; }

u32 UUID::variant() const noexcept { return _data[0x8] >> 6; }

bool UUID::invalid() const noexcept { return _data == _cachedInvalid._data; }

UUID::operator bool() const noexcept { return invalid(); }

std::strong_ordering UUID::operator<=>(const UUID& other) const noexcept {
  return _data <=> other._data;
}

const std::array<u8, 16>& UUID::data() const noexcept { return _data; }

std::string UUID::toString() const noexcept {
  std::string result;
  result.resize(37, ' ');

  std::sprintf(result.data(),
               "%02hhx%02hhx%02hhx%02hhx-%02hhx%02hhx-%02hhx%02hhx-%02hhx%"
               "02hhx-%02hhx%02hhx%02hhx%02hhx%02hhx%"
               "02hhx",
               _data[0x0], _data[0x1], _data[0x2], _data[0x3], _data[0x4],
               _data[0x5], _data[0x6], _data[0x7], _data[0x8], _data[0x9],
               _data[0xa], _data[0xb], _data[0xc], _data[0xd], _data[0xe],
               _data[0xf]);
  result.pop_back();

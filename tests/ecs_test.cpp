...
namespace arch::ecs {
	TEST(ECSTest, MovingObjects) {
		...
		for (auto i: entity_ids)
			ASSERT_FLOAT_EQ(coordinator.get_component<Transform>(i).position.x, 1.0f);
		...
		for (auto i : entity_ids)
			ASSERT_FLOAT_EQ(coordinator.get_component<Transform>(i).position.x, 3.0f);
		ASSERT_FLOAT_EQ(coordinator.get_component<Transform>(new_entity_id).position.x, 2.0f);
	}
}